...
class RandoConverter implements Converter {
    ...
    public Object fromBody(TypedInput typedInput, Type type) throws ConversionException {
        try {
            ...
            Log.d("error converting KML", e.getLocalizedMessage());
    ...
    private Rando readKml(XmlPullParser parser) throws XmlPullParserException,
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            ...
            if (name.equals("name")) {
                ...
                // Process the segment or beacon location
                isBeacon = false;
                while (parser.next() != XmlPullParser.END_TAG) {
                    if (parser.getEventType() != XmlPullParser.START_TAG) {
                        continue;
                    ...
                    if (subName.equals("description")) {
                        String descr = readText(parser);
                        isAller = "Segment 1".equals(descr);
                    } else if (subName.equals("LineString")) {
                        currentSegment = readLineString(parser);
                    } else if (subName.equals("Point")) {
                        //process beacon
                        isBeacon = true;
                        if (mostRecentBeacon == null) {
                            //first beacon occurrence, should be the most recent one
                            parser.next();
                            if (parser.getName().equals("coordinates")) {
                                List<LatLng> coords = readCoords(parser);
                                if (coords.size() > 0) {
                                    mostRecentBeacon = coords.get(0);
                                }
                            }
                        }
                    } else {
        ...
        rando.setLastRandoPosition(mostRecentBeacon);
